...
%% Caracterización de fuerza del sensor. Vía mecánica
...
% Descartar puntos que tengan una relación Fuerza/Deformación incoherente
...
fmecanica = log10(fuerza_val_m);
[xData, yData] = prepareCurveData( deformacion_val, fuerza_val_m );

% Set up fittype and options.
ft = fittype( 'poly2' );
% excludedPoints = (yData < 1.5) | (yData > 16);
...
% Label axes
xlabel( 'Desplazamiento medio de los marcadores (px)', 'Interpreter', 'none' );
ylabel( 'Fuerza (N)', 'Interpreter', 'none' );
legend('Datos','Datos excluídos','Recta ajustada','Location', 'northwest')
grid on
title('R^2=',gof.rsquare);

coeficientes= coeffvalues(fitresult); 
p1 = coeficientes(1); % Coeficiente del término cuadrático (x^2)
p2 = coeficientes(2); % Coeficiente del término lineal (x)
p3 = coeficientes(3); % Término independiente (intersección)

%% Caracterización de fuerza del sensor. Vía óptica (Brillo medio)
...
[xData, yData] = prepareCurveData( brillo_medio_val, fuerza_val_o_medio );
...
h = plot( fitresult, xData, yData, excludedPoints);
legend('Datos','Datos excluídos','Recta ajustada','Location', 'northwest')
xlabel( 'Brillo medio en la zona de contacto (GL)', 'Interpreter', 'none' );
ylabel( 'Fuerza (N)', 'Interpreter', 'none' );
grid on
title('R^2=',gof.rsquare);

coeficientes_medio = coeffvalues(fitresult); 
p1 = coeficientes_medio(1); % Coeficiente del término cuadrático (x^2)
p2 = coeficientes_medio(2); % Coeficiente del término lineal (x)
p3 = coeficientes_medio(3); % Término independiente (intersección)


%% Caracterización de fuerza del sensor. Vía óptica (Brillo máximo)
...
[xData, yData] = prepareCurveData( brillo_maximo_val, fuerza_val_o_maximo );
...
h = plot( fitresult, xData, yData, excludedPoints);
legend('Datos','Datos excluídos','Recta ajustada','Location', 'northwest')
xlabel( 'Brillo máximo en la zona de contacto (GL)', 'Interpreter', 'none' );
ylabel( 'Fuerza (N)', 'Interpreter', 'none' );
grid on
title('R^2=',gof.rsquare);

coeficientes_max = coeffvalues(fitresult); 
p1 = coeficientes_max(1); % Coeficiente del término cuadrático (x^2)
p2 = coeficientes_max(2); % Coeficiente del término lineal (x)
p3 = coeficientes_max(3); % Término independiente (intersección)
...
%% Calibración de las semiesferas blandas. Radio vs constante elástica

constantes=[1.2223 1.3442 1.3681 1.6421 2.3474 4.3168];

CirculoManual20=[147 142.13 149.23 153 156.3 164.47];

CirculoManual30=[114.77 123 127.31 137.2 145.66 153.67];

figure;
plot(constantes,CirculoManual20)
hold on
plot(constantes, CirculoManual20, 'o')
ylim([110 170])
xlim([0 4.5])
grid on
ylabel('Radio del círculo (píxeles)')
xlabel('Constante elástica (N/mm)')

figure;
plot(constantes,CirculoManual30)
hold on
plot(constantes, CirculoManual30, 'o')
ylim([110 170])
xlim([0 4.5])
grid on
ylabel('Radio del círculo (píxeles)')
...
%% Calibración de las semiesferas blandas. Brillo vs constante elástica

BrilloMedio20=[18.09 18.73 18.3 20.49 21.71 27.22];
BrilloMax20=[113.96 119.29 111.88 114.15 120.16 127.88];

figure;
plot(constantes,BrilloMedio20)
hold on
plot(constantes, BrilloMedio20, 'o')
ylim([0 30])
xlim([0 4.5])
grid on
ylabel('Brillo medio en la zona de contacto (GL)')
xlabel('Constante elástica (N/mm)')

BrilloMedio30=[18.84 18.85 18.98 19.09 19.32 24.73];
BrilloMax30=[97.05 103.06 105.04 106.19 108.74 129.83];

figure;
plot(constantes,BrilloMedio30)
hold on
plot(constantes, BrilloMedio30, 'o')
ylim([0 30])
xlim([0 4.5])
grid on
ylabel('Brillo medio en la zona de contacto (GL)')
